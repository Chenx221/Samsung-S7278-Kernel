...
#if 1 /* It seems kernel has bug. So add workaround patch */
			mt = get_pageblock_migratetype(page);
			set_freepage_migratetype(page, mt);
#else /* here is original linux kernel source */
				...
				struct zone *zone, unsigned long zonesize)
{
	...
	if (usemapsize)
		zone->pageblock_flags = alloc_bootmem_node_nopanic(pgdat,
								   usemapsize);
	printk("pageblock_flags is init as 0x%x, for zone 0x%x\n", zone->pageblock_flags, zone);
}
#else
					...
					int start_bitidx, int end_bitidx)
{
	...
	if(bitmap == NULL) {
		printk("page 0x%x, zone 0x%x with a NULL bitmap\n", page, zone);
	}
