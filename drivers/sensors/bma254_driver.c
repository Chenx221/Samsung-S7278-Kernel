...
#endif
//#include <linux/gpio.h>
//#include <mach/gpio-edge.h> //Marvell pxa1088
...
static void bma254_work_func(struct work_struct *work)
{
	...
	ret = bma254_read_accel_xyz(bma254, &acc);
	if(ret >= 0) {
		...
		const char *buf, size_t count)
{
	...
	printk(KERN_INFO "ACC] %s : delay data[%d]", __func__, data);
...
static void bma254_set_enable(struct device *dev, int enable)
{
	...
	printk(KERN_INFO "ACC] %s : enable[%d]", __func__, enable);
		...
		const char *buf, size_t count)
{
	...
	if(data){
		...
		if (bma254_set_offset_target_x(bma254->bma254_client, (unsigned char)0) < 0)
			return -EINVAL;
		...
		if (bma254_set_offset_target_y(bma254->bma254_client, (unsigned char)0) < 0)
			return -EINVAL;
