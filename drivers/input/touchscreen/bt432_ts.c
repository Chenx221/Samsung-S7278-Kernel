...
#define MAX_RAW_DATA_SZ				180 /* 18x10*///448 /* 28x16*///576 /* 32x18 */
...
#define TOUCH_REGULATOR_NAME "vddcammot"
#define TOUCH_REGULATOR_VOLTAGE 3000000
...
retry_init:
	...
	if (!zinitix_bit_test(chip_eeprom_info, 0))
	{
		if (write_reg(client,0x0168, 26) != I2C_SUCCESS)
		    goto fail_init;
		if (write_reg(client,0x0060, 20) != I2C_SUCCESS)
		    goto fail_init;
	}
#endif
		...
		const struct i2c_device_id *i2c_id)
{
	...
	if (ret < 0) {
		pr_err("bt432: Failed to obtain gpio for iID\n");
		return -1;
	}
	if(!gpio_get_value(TSP_ID_VALID)){
		pr_info("TSP ID VAILD is %d\n", gpio_get_value(TSP_ID_VALID));
...
#else
	m_firmware_data = m_firmware_data_12;
	pdata->page_size = 64;
	info->cap_info.ic_fw_size = 24 * 1024;
#endif
...
/*	input_set_abs_params(info->input_dev, ABS_MT_WIDTH_MINOR,
	...
	ret = request_threaded_irq(info->irq, NULL, bt432_touch_irq_handler,
		IRQF_TRIGGER_LOW | IRQF_ONESHOT , info->input_dev->name, info);
