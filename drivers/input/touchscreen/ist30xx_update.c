...
int ist30xx_check_auto_update(struct ist30xx_data *data)
{
	...
	if (data->chip_id == IST30XXB_CHIP_ID) {
		while (retry--) {
			...
			if (ret == 0) {
				if ((ist30xx_tsp_type == 0) || (ist30xx_tsp_type == 1) || (ist30xx_tsp_type == 0xf) || (ist30xx_tsp_type == 0xe) || (ist30xx_tsp_type == 0x7))
					tsp_check = true;
				tsp_info("tsp type: %x\n", ist30xx_tsp_type);
				break;
			}
	...
	ist30xx_read_cmd(data->client,
			 CMD_GET_TSP_PANNEL_TYPE, &ist30xx_tsp_type);
	printk("[TSP]%x\n",ist30xx_tsp_type);
	if(ist30xx_tsp_type == 0xffffffff) return -2;
...
#if 0//def CONFIG_MACH_LOGAN
		if(data->param_ver != 0x00000001)
			goto fw_check_end;
#endif
		if ((data->fw.ver >= ist30xx_fw_ver) &&
		    (data->param_ver >= ist30xx_param_ver))
...
static DEVICE_ATTR(param, S_IWUSR | S_IWGRP, NULL, ist30xx_param_store);
