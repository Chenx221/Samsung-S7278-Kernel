...
#include "ist30xx_fw_kyle-ve.h"
#include "ist30xxb_fw_logan.h"
#else
#include "ist30xx_fw_kyle-ve.h"
#include "ist30xxb_fw_kyle-ve.h"
#endif
#endif
...
{
#if IST30XX_ZVALUE_MODE
	ist30xx_write_cmd(data->client, CMD_ZVALUE_MODE, 2);
	msleep(10);
	ist30xx_write_cmd(data->client, CMD_SAME_POSITION, 1);
	msleep(10);
#endif
#if IST30XX_DETECT_TA
...
extern int set_cpufreq_min_limit(int freq);
#endif
	...
	if (press == PRESS_MSG_MASK) {
		if (tsp_touched[idx] == 0) { // touch down
			...
			tsp_info("%s - %d (%d, %d), %d\n",
				 ...
				 finger->bit_field.x, finger->bit_field.y, ist30xx_zvalue);
		} else {                    // touch move
			tsp_verb("%s   %d (%d, %d), %d\n",
...
static void report_input_data(struct ist30xx_data *data, int finger_counts, int key_counts)
{
	...
	for (i = finger_counts; i < finger_counts + key_counts; i++) {
		...
		if (key_id == 1)
		    ist30xx_sensitivity_key1 = (key_press ? ist30xx_key_sensitivity : 0);
		else if (key_id == 2)
		    ist30xx_sensitivity_key2 = (key_press ? ist30xx_key_sensitivity : 0);
	}
...
#if IST30XX_ZVALUE_MODE
    ret = ist30xx_get_position(data->client, &ist30xx_zvalue, 1);
    if (ret)
        goto irq_err;
#endif
...
void get_fw_ver_bin(void *device_data)
{
	...
	set_default_result(info);
#if IST30XX_INTERNAL_BIN
	if(info->chip_id == IST30XXB_CHIP_ID)
	{
		if(info->tsp_type== 1)
			snprintf(buff, sizeof(buff), "IM01%04x", ist30xx_parse_param_ver(PARSE_FLAG_FW, ist30xxb_fw));
		else
			snprintf(buff, sizeof(buff), "IM00%04x", ist30xx_parse_param_ver(PARSE_FLAG_FW, ist30xxb_fw));
	}
	else
	{
		if(info->tsp_type== 1)
...
void get_threshold(void *device_data)
{
	struct ist30xx_data *info = (struct ist30xx_data *)device_data;
	char buff[16] = {0};
	int v_threshold=350; //touch_key = 300
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	if(data->chip_id == IST30XXB_CHIP_ID)
		tsp_fw_version= ist30xx_parse_param_ver(PARSE_FLAG_FW, ist30xxb_fw);
	else
...
#ifdef CONFIG_MACH_LOGAN
	ret = gpio_request(TSP_ID_VALID, "TSP_ID_VALID");
	if (ret < 0) {
		tsp_info("Failed to obtain gpio for ID\n");
		return -1;
	}
	if(gpio_get_value(TSP_ID_VALID)) {
		tsp_info("TSP ID VAILD is %d\n", gpio_get_value(TSP_ID_VALID));
		gpio_free(TSP_ID_VALID);
	}
	else {
		gpio_free(TSP_ID_VALID);
		return -1;
