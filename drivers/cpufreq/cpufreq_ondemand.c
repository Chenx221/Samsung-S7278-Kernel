...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		cpufreq_debug("****DVFS**** %s: cpu=%u,load=%u,freq_avg=%d,policy->cur=%u\n",__func__,j,load,freq_avg,policy->cur);
		if (freq_avg <= 0)
			freq_avg = policy->cur;
	...
	cpufreq_debug("****DVFS**** %s: memfreq_increase_cpu_frequency = %d\n",__func__,memfreq_increase_cpu_frequency);
...
static void dbs_refresh_callback(struct work_struct *unused)
{
	...
	if (policy->cur < policy->max) {
		...
		cpufreq_debug("****DVFS**** %s: cpufreq increase\n",__func__);
