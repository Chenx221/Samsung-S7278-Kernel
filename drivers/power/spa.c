...
#define SPA_TOTAL_CHARGING_TIME_LIMIT        (60*60*5)
#define SPA_BACK_CHARGING_TIME_LIMIT        (60*40)
#define SPA_RESTART_CHARGING_TIME_LIMIT        (60*90)
#define SPA_RESTART_CUT_OFF_TIME_LIMIT		(60*15)
u32 totalcharging_time = 0;
u32 backcharging_time = 0;
u32 restartcharging_time = 0;

#define SPA_STANDBY_CHARGING_THRESHOLD_HIGH        135
#define SPA_STANDBY_CHARGING_THRESHOLD_LOW        6
...
#if defined(CONFIG_TOUCHSCREEN_MMS134S_TS) || defined(CONFIG_TOUCHSCREEN_IST30XX)
...
#elif defined (CONFIG_MACH_KYLEVE)
        static s16 a_TempAdcTable[] = {
		3632,
		3298,
		3194,
...
#elif defined (CONFIG_MACH_LOGAN)
        static s16 a_TempAdcTable[] = {
                3632,
                3298,
                3194,
...
static void spa_monitor_voltage_work(struct work_struct *work)
{
	...
	printk("read_soc=%d, battery.soc=%d, average_voltage=%d,"
	       ...
	       info->battery.health, info->battery.status,
	       info->battery.current_temp_adc,
	       info->battery.current_temperature/1000,
	       info->battery.end_of_charge);
	...
	if(soc <= 0)
	{
		...
		else
		{
			...
			printk("%s : JIG = %d, VF = %d \n", __func__,
			       factory_exception_jig_connected,
			       factory_exception_batt_VF);
			printk("%s : SOC = %d, VOLT = %d \n", __func__,
			       soc, spa_bat_get_average_voltage(info));

			if( (factory_exception_jig_connected == 1) &&
			    (factory_exception_batt_VF == 0))
			{
	...
	if(vBatValueIndex == SPA_BAT_VOLTAGE_SAMPLE)
	{
		...
		if (current_charger_type == POWER_SUPPLY_TYPE_BATTERY)
		{
		...
		else if ((current_charger_type == POWER_SUPPLY_TYPE_MAINS) ||(current_charger_type == POWER_SUPPLY_TYPE_USB))
		{
			if (soc < info->battery.soc)
			{
			...
			else
			{
				if((batt_VF < info->battery.VF_low) || (batt_VF > info->battery.VF_high))
				{
					...
					else
					{
						...
						{
							msleep(1000);
							spa_bat_read_voltage(info, &vbat);
							ret = spa_bat_read_VF(info, &batt_VF);
							...
							if ((ret >= 0) && (batt_VF >= info->battery.VF_low) && (batt_VF <= info->battery.VF_high))
							{
								iVFExist = 1;
							}
						}
	...
	if((current_charger_type ==POWER_SUPPLY_TYPE_MAINS) ||(current_charger_type ==POWER_SUPPLY_TYPE_USB))
	{
		...
		if (info->pdata->get_charger_full_status &&
			info->pdata->get_charger_full_status() &&
		...
		if (info->battery.soc == 100) {
			if (eChargingState == SPA_STATE_BACK_CHARGING) {
				charge_limited_time = backcharging_time;
				cut_off_time = SPA_BACK_CHARGING_TIME_LIMIT;
			}
#if defined(CONFIG_MACH_KYLEVE) || defined(CONFIG_MACH_LOGAN)
			else if (eChargingState == SPA_STATE_RESTART_CHARGING) {
				charge_limited_time = restartcharging_time;
				cut_off_time = SPA_RESTART_CUT_OFF_TIME_LIMIT;
			}

			if (eChargingState != SPA_STATE_NORMAL_CHARGING) {
#endif
				u32 temp = get_sys_cnt() /1000;
				if (temp - charge_limited_time > cut_off_time)
				{
					printk("read_soc, 111, in %s charging state,"
						" beginCnt=%d, EndCnt=%d,  stop charging.\n",
						eChargingState == SPA_STATE_BACK_CHARGING ? "Back" : "Restart",
						charge_limited_time, temp);

					eChargingState = SPA_STATE_STANDBY;
					spa_bat_set_end_of_charge(info, SPA_END_OF_CHARGE_BY_FULL);
					spa_chg_stop_charge(info, SPA_END_OF_CHARGE_BY_FULL);
				}
#if defined(CONFIG_MACH_KYLEVE) || defined(CONFIG_MACH_LOGAN)
		...
		if ((info->battery.soc ==100)
		    && (eChargingState == SPA_STATE_RESTART_CHARGING)
		    && (nCurrentChecking1< SPA_STANDBY_CHARGING_THRESHOLD_HIGH)
		    && (nCurrentChecking1>= SPA_STANDBY_CHARGING_THRESHOLD_LOW)
			...
			||(eChargingState == SPA_STATE_BACK_CHARGING))
		{
			...
			{
				if (info->battery.current_voltage > SPA_CHARGING_RESTART_VOLTAGE)
				{
					eChargingState = SPA_STATE_STANDBY;
					spa_bat_set_end_of_charge(info, SPA_END_OF_CHARGE_BY_FULL);
					spa_chg_stop_charge(info, SPA_END_OF_CHARGE_BY_FULL);
...
int spa_chg_register_enable_charge(void (*enable_charge)(enum power_supply_type))
{
	...
	if(spa_chg_get_current_charger(ginfo) == SPA_CHARGER_TYPE_TA) {
		...
		enable_charge(POWER_SUPPLY_TYPE_USB_DCP);
#else
...
static void spa_bat_recharge_start_timer_work(struct work_struct *work)
{
	...
	if(spa_bat_check_end_of_charge(info, SPA_END_OF_CHARGE_BY_TIMER) == 0)
	{
		...
		if(end_of_charge == SPA_END_OF_CHARGE_NONE)
		{
			if(spa_bat_get_average_voltage(info) < SPA_CHARGING_RESTART_VOLTAGE)
			{
				printk("%s restart charge because voltage is lower than %dmv\n", __func__, SPA_CHARGING_RESTART_VOLTAGE);
				spa_chg_start_charge(info, SPA_CHARGE_RESTART_TIMER);
			}
