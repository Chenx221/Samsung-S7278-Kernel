...
#if defined(CONFIG_MACH_GARDA) || defined(CONFIG_MACH_KYLEVE) || defined(CONFIG_MACH_LOGAN)
	gpio_request(EIC_KEY_POWER_2, "gpint2"); 
	gpio_direction_input(EIC_KEY_POWER_2); 
	jig_on = gpio_get_value(EIC_KEY_POWER_2); 
#else
        int ret;
 ...
 */
#if defined(CONFIG_SPA) 
extern int spa_check_battery(void);
#endif
#define VBAT_BUFF_NUM	7
int battery_updata(void)
{
#if defined(CONFIG_SPA) 
	return spa_check_battery();
#else
	int32_t adc_value;
	int32_t voltage;
	uint32_t capacity;
