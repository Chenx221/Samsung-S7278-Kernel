...
#define STC311x_CC_CNF_CH	313
#define STC311x_CC_CNF_DISCH  290
  ...
  //early empty compensation
    ...
    if (BattData.AvgVoltage<APP_MIN_VOLTAGE)
		...
		if (BattData.AvgVoltage<(APP_MIN_VOLTAGE+200))
		{
		  BattData.SOC = BattData.SOC * (BattData.AvgVoltage - APP_MIN_VOLTAGE) / 200;
		  /*
		  if(BattData.SOC < 80 && BattData.GG_Mode == CC_MODE)
		  {
			STC311x_ForceVM();
		  }
		  */
...
#ifndef CONFIG_MACH_NEVISTD
	int value;
	//sense resistor compensation
	value = STC31xx_ReadWord(STC311x_REG_CC_CNF);
	if( BattData.BattState<=0)	//discharging
        {
		if( value != STC311x_CC_CNF_DISCH )
			STC31xx_WriteWord(STC311x_REG_CC_CNF,STC311x_CC_CNF_DISCH); 
        }
	else	//charging
        {
		if( value != STC311x_CC_CNF_CH )
			STC31xx_WriteWord(STC311x_REG_CC_CNF,STC311x_CC_CNF_CH); 
        }
#endif
...
#else
      if(BattData.BattState > BATT_IDLE && BattData.SOC >= 990 && BattData.SOC < 995 && BattData.AvgCurrent > 500)
#endif
      {
        BattData.SOC = 990;
...
//	printk("read_voltage enter\n");
	...
	if (res>0) 
	{
	    ...
	    extern int nCurrentChecking1;
	    extern int nCurrentChecking2;
	    extern int nCurrentChecking3;		

	    // copy check data.
	    nCurrentChecking1 = nCurrentChecking2;
	    nCurrentChecking2 = nCurrentChecking3;
	    nCurrentChecking3 = GasGaugeData.Current;		
#endif 		
	}
	else if(res == -1)
	{
...
#if 0 //defined(CONFIG_MACH_NEVISTD)  
...
/* stc chip reset */
  ...
  printk("stc311x probe debug 3\n");
	  	...
	  	GasGaugeData.ForceExternalTemperature = chip->pdata->ForceExternalTemperature; /* 1=External temperature, 0=STC3115 temperature */
#else
	  	GasGaugeData.ExternalTemperature = 25; /*External temperature fonction, return C*/
	  	GasGaugeData.ForceExternalTemperature = chip->pdata->ForceExternalTemperature; /* 1=External temperature, 0=STC3115 temperature */
#endif
	}
