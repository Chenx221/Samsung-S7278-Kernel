...
#endif
#if defined(CONFIG_RTC_CHN_ALARM_BOOT)	
static int sprd_rtc_shutdown_flag;
static int sprd_rtc_shutdown_start_flag;
#endif
...
static int sprd_rtc_set_sec(unsigned long secs)
{
	...
	if(day != get_day()){
#if defined(CONFIG_ARCH_SC8825)
		u32 flag=sci_adi_read(ANA_RTC_DAY_CNT)&0xC000;
		sci_adi_raw_write(ANA_RTC_DAY_UPDATE, flag|(day&0x3FFF));
#else
...
#if defined(CONFIG_ARCH_SC8825)
static u16 sprd_rtc_get_alarm_flag(void)
{
	u32 day = sci_adi_read(ANA_RTC_DAY_ALARM_CNT);
	u16 flag=(day&0xC000)>>14;
	printk("alarm:flag=%x,day=%x\n",flag,day);
	return flag;
}
#endif
...
#if defined(CONFIG_ARCH_SC8825)
/*value ranges from:1,2,3*/
static int sprd_rtc_set_alarm_flag(u16 value)
{
	u32 int_rsts = 0;
	u32 set_mask =0;
	int i=0;
	...
	if(value > 3)
		return 1;
	sci_adi_set(ANA_RTC_INT_CLR, RTC_DAY_ALM_ACK_BIT|RTC_DAY_ACK_BIT);

	u32 day=sci_adi_read(ANA_RTC_DAY_ALARM_CNT);
	u32 flag = (value << 14)&0xC000;
	if (flag != (day&0xC000))
	{
		sci_adi_raw_write(ANA_RTC_DAY_ALM, flag|(day&0x3FFF));
		set_mask |= RTC_DAY_ALM_ACK_BIT;
		set_flag=1;
	}
	day=sci_adi_read(ANA_RTC_DAY_CNT);
	flag = (value << 14)&0xC000;
	if (flag != (day&0xC000))
	{
		sci_adi_raw_write(ANA_RTC_DAY_UPDATE, flag|(day&0x3FFF));
		set_mask |= RTC_DAY_ACK_BIT;
	...
	if(set_flag=0)
		...
		rtc_mask = RTC_DAY_ALM_ACK_BIT;
	else if(set_flag=2)
		rtc_mask = RTC_DAY_ALM_ACK_BIT|RTC_DAY_ACK_BIT;
	else if(set_flag=3)
		rtc_mask = RTC_DAY_ACK_BIT;
	...
	do{
		int_rsts = sci_adi_read(ANA_RTC_INT_RSTS) & rtc_mask;
	...
	printk("sprd_rtc_set_alarm_flag done value : [%d]\n", value);
	return 0;
}
#endif
...
#if defined(CONFIG_ARCH_SC8825)
	u32 flag=sci_adi_read(ANA_RTC_DAY_ALARM_CNT)&0xC000;
	sci_adi_raw_write(ANA_RTC_DAY_ALM, flag|(day&0x3FFF));
#else
...
#endif
		{
			printk("sprd_rtc_set_alarm lock rtc\n");
...
#if defined(CONFIG_ARCH_SC8825)
#if defined(CONFIG_BATTERY_SAMSUNG)
	if(lpcharge && (sprd_rtc_get_alarm_flag() & SPG_CNT_ALARM_BOOT))
#else
	if(spa_lpm_charging_mode_get() && (sprd_rtc_get_alarm_flag() & SPG_CNT_ALARM_BOOT))
#endif
#else
#if defined(CONFIG_BATTERY_SAMSUNG)
	if(lpcharge)
...
static int read_current_rtc_alarm(void)
{
	unsigned int sec, min, hour, day;
	struct rtc_time tm;
	unsigned int temp;

	temp = sci_adi_read(ANA_RTC_SPG_CNT);
	sprd_rtc_read_time(NULL, &tm);
	sec = sci_adi_read(ANA_RTC_SEC_ALARM_CNT);
	min = sci_adi_read(ANA_RTC_MIN_ALARM_CNT);
	hour = sci_adi_read(ANA_RTC_HRS_ALARM_CNT);
#if defined(CONFIG_ARCH_SC8825)
	sprd_rtc_get_alarm_flag();
	day = sci_adi_read(ANA_RTC_DAY_ALARM_CNT)& RTC_DAY_MASK;
#else
	day = sci_adi_read(ANA_RTC_DAY_ALARM_CNT);
#endif
	printk("read_current_rtc_alarm  spg [0x%x] time => day : %d, hour : %d, min : %d\n", temp, tm.tm_mday, tm.tm_hour, tm.tm_min); 
	printk("read_current_rtc_alarm  alarm => day : %d hour : %d min : %d sec : %d\n", day, hour, min, sec);
...
#if defined(CONFIG_RTC_CHN_ALARM_BOOT)
	sprd_rtc_shutdown_flag = 0;
	sprd_rtc_shutdown_start_flag = 0;
...
#if defined(CONFIG_RTC_CHN_ALARM_BOOT)
	sprd_rtc_shutdown_start_flag = 1;

	sprd_rtc_set_alarm(&pdev->dev, &autoboot_alm_exit);
#if defined(CONFIG_ARCH_SC8825)
	sprd_rtc_set_alarm_flag(SPG_CNT_ALARM_BOOT);
#else
