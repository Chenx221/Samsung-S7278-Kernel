...
#if defined(CONFIG_SEC_GPIO_DVS)
#if defined (CONFIG_MACH_LOGANU)
 extern void dvs_set_init();
#endif
...
#if defined (CONFIG_ARCH_SC8825) && defined (CONFIG_MACH_LOGAN)
	/* over voltage check*/
	if (battery->voltage_now > SEC_BATTERY_OVER_VOLTAGE) {
		dev_err(battery->dev,
			"Over voltage detect, voltage_now is %d, powering off ...\n",
			battery->voltage_now);
		if (pm_power_off)
			pm_power_off();
	}
...
static int __devinit sec_battery_probe(struct platform_device *pdev)
{
	...
	if (bttery->pdata->is_lpm && battery->pdata->is_lpm()) {
