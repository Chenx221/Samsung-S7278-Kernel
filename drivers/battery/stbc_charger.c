...
static int STBCFG01_BatChg_Read_Ints(struct i2c_client *client) {
	...
	// clear status
	res = STBCFG01_WriteByte(client, STBCFG01_REG_CHG_INT_LATCH1, 0x0000);
	if (res<0)
		return(res);
...
static void STBCFG01_ChargerControl(struct sec_charger_info *charger)
{
	...
	res = STBCFG01_ReadByte(charger->client, STBCFG01_REG_CTRL);
	res = res >> 2;
...
bool sec_hal_chg_init(struct sec_charger_info *charger)
{
	...
	if (ChargerData.GPIO_cd) {
		...
		printk("STBCFG01 : SHDN pin not assigned in platformdata, will not be used\n");
	}
