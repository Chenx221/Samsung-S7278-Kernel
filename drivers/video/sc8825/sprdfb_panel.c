...
int32_t sprdfb_panel_set_brightness(uint16_t brightness)
{
	struct sprdfb_device *dev = g_dev;
	struct panel_spec *panel;
	struct panel_operations *panel_drv;
	...
	if (!dev->panel || !dev->panel->ops) {
		printk("%s, panel is null\n", __func__);
		return 0;
	}

	if (!dev->panel_ready) {
		printk("%s, panel is not ready to control\n", __func__);
		return 0;
	}

	panel = dev->panel;
	panel_drv = dev->panel->ops;

	if (panel_drv->panel_set_brightness)
		panel_drv->panel_set_brightness(panel, brightness);
