...
#define VIBRATOR_STABLE_LEVEL   (10)
...
#if 0//defined(CONFIG_MACH_GARDA) || defined(CONFIG_MACH_KYLEVE) || defined(CONFIG_MACH_LOGAN)
#define VIBRATOR_GPIO_CONTROL
#define GARDA_VIB_GPIO 172
#endif
...
#ifdef VIBRATOR_GPIO_CONTROL
	if (on)
		gpio_direction_output(GARDA_VIB_GPIO,1);
	else
		gpio_direction_output(GARDA_VIB_GPIO,0);
#endif
}
...
#ifdef VIBRATOR_GPIO_CONTROL /*gpio request*/
	if(gpio_request(GARDA_VIB_GPIO, "vib_gpio"))
		printk(KERN_ERR "Vibrator Request GPIO%d failed!\n", GARDA_VIB_GPIO);
	else
		printk(KERN_ERR "Vibrator Request GPIO%d Succeeded!\n",GARDA_VIB_GPIO);
	gpio_direction_output(GARDA_VIB_GPIO,0);
#endif
