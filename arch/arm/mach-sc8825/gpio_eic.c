...
static int sci_gpio_get_config(struct gpio_chip *chip, unsigned offset)
{
	pr_debug("%s %d+%d\n", __FUNCTION__, chip->base, offset);
	return sci_gpio_read(chip, offset, REG_GPIO_DIR);
}
#endif
...
#ifdef CONFIG_SEC_GPIO_DVS
	.chip.get_config	= sci_gpio_get_config,
#endif
