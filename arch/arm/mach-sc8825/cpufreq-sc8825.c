...
static int cpufreq_set_freq_vdd_with_notify(unsigned long mcu_clk,unsigned long mcu_vdd);
...
static void set_dcdcarm(unsigned long vddarm)
{
	...
	cpufreq_debug("***** before, ANA_REG_GLB_DCDCARM_CTRL0:0x%x ****\n", val );
#endif
...
#ifdef CONFIG_CPU_FREQ_DEBUG
	cpufreq_debug("**** after, cpu:%d, ANA_REG_GLB_DCDCARM_CTRL0:0x%x ****\n",
			cpu, sci_adi_read(ANA_REG_GLB_DCDCARM_CTRL0) );
	cpufreq_debug("**** after, cpu:%d, ANA_REG_GLB_DCDCARM_CTRL_CAL:0x%x ****\n",
...
static void set_mcu_clk_freq_div(u32 cpu, u32 mcu_freq)
{
	...
	arm_clk_div -= 1;
#ifdef CONFIG_CPU_FREQ_DEBUG
	cpufreq_debug("before, AHB_ARM_CLK:%08x \n", __raw_readl(REG_AHB_ARM_CLK) );
#endif
...
#ifdef CONFIG_CPU_FREQ_DEBUG
	cpufreq_debug("after, AHB_ARM_CLK:%08x, val:%x, div = %d\n",
...
static void set_mcu_clk_freq(u32 cpu, u32 mcu_freq)
{
	...
	gr_gen1 |= BIT(9);
	__raw_writel(gr_gen1, GR_GEN1);
	cpufreq_debug("before, mpll_cfg:0x%x, mpll_n:%u, mpll_refin:%u\n", __raw_readl(GR_MPLL_MN), mpll_n, mpll_refin);
	__raw_writel(mpll_cfg, GR_MPLL_MN);
	gr_gen1 &= ~BIT(9);
	__raw_writel(gr_gen1, GR_GEN1);

	sc8825_wait_mpll();
	cpufreq_debug("after, mpll_cfg:0x%x, mpll_n:%u, mpll_refin:%u\n", __raw_readl(GR_MPLL_MN), mpll_n, mpll_refin);
#else
				...
				unsigned long mcu_vdd)
{
	...
	printk("****DVFS**** %s: cpu=%lu,old=%lu,new=%lu\n",__func__,cpu,current_cfg.clk_mcu_mhz,mcu_clk);
	...
	for_each_possible_cpu(i) {
		...
		if(policy_update){
			...
			printk("cpu_set_freq_vdd policy_update error!!!\n");
		}
	}
#endif
...
static int sprd_cpufreq_set_rate(struct cpufreq_policy *policy, int index)
{
	...
	cpufreq_debug("****DVFS**** %s: cpufreq_bypass=%d\n",__func__,cpufreq_bypass);
...
static unsigned int sprd_cpufreq_get_speed(unsigned int cpu)
{
	...
	return scalable_sc8825[0].dvfs_tbl[0].clk_mcu_mhz;
#endif
}
				...
				unsigned int relation)
{
	...
	if(!table){
		printk("cpufreq: sprd_cpufreq_set_target get_table error!!!\n");
		//ret = -EFAULT;
		//goto done;
		table = scalable_sc8825[policy->cpu].freq_tbl;
	}
...
static int sprd_cpufreq_driver_init(struct cpufreq_policy *policy)
{
	...
	for(cnt = 0; cnt < FREQ_TABLE_ENTRY-1; cnt++)
	{
		if(current_cfg.clk_mcu_mhz==scalable_sc8825[0].dvfs_tbl[cnt].clk_mcu_mhz){
			current_cfg.vdd_mcu_mv=scalable_sc8825[0].dvfs_tbl[cnt].vdd_mcu_mv;
			break;
		}
	}
...
static int sprd_cpufreq_suspend(void)
{
	...
	cpufreq_set_freq_vdd_with_notify(dvfs_tbl[0].clk_mcu_mhz,dvfs_tbl[0].vdd_mcu_mv);
				...
				unsigned long mcu_vdd)
{
	...
	if(mcu_clk > cur_clk){
		...
		ret = 1;
		cpufreq_debug("return 1 \n");
		return ret;
	}

	printk("****DVFS**** %s: cpu=0,old=%lu,new=%lu\n",__func__,current_cfg.clk_mcu_mhz,mcu_clk);
	...
	for_each_possible_cpu(i){
		...
		if(policy_update){
			...
			printk("cpufreq_set_freq_vdd policy_update error!!!\n");
		}
	}
#endif
...
static int cpufreq_set_freq_vdd_with_notify(unsigned long mcu_clk,
				unsigned long mcu_vdd)
{
	unsigned int ret;
	struct cpufreq_freqs freqs;
	...
	cpufreq_debug("****DVFS**** %s: freqs.old=%u,freqs.new=%u\n",__func__,freqs.old,freqs.new);
	if(freqs.new==freqs.old){
		return 1;
	}
	...
	ret = cpufreq_set_freq_vdd(mcu_clk,mcu_vdd);
	mutex_unlock(&cpufreq_mutex);
	if(ret>0){
		cpufreq_debug("cpufreq not need to set 0\n");
	}else{
		freqs.old = cpufreq_freq_old;
		for_each_possible_cpu(cpu) {
			freqs.cpu = cpu;
...
static ssize_t cpufreq_show(struct kobject *kobj, struct kobj_attribute *attr,
			  char *buf)
{
	if(!current_cfg.clk_mcu_mhz){
		return sprintf(buf,"%lumhz,%lumv\n",scalable_sc8825[0].dvfs_tbl[0].clk_mcu_mhz/1000,scalable_sc8825[0].dvfs_tbl[0].vdd_mcu_mv/1000);
	}
...
{
#ifdef CONFIG_CPU_FREQ_SC8825_FIXED_FREQ
	/*Only cpu0*/
	struct sprd_dvfs_table *dvfs_tbl = scalable_sc8825[0].dvfs_tbl;
	unsigned int cpufreq_choose = 0;

	switch(buf[0]) {
	case '0':
		cpufreq_debug("****DVFS**** %s: 0\n",__func__);
		cpufreq_choose = 0;
		cpufreq_set_freq_vdd_with_notify(dvfs_tbl[cpufreq_choose].clk_mcu_mhz,dvfs_tbl[cpufreq_choose].vdd_mcu_mv);
		break;
	case '1' :
		cpufreq_debug("****DVFS**** %s: 1\n",__func__);
		cpufreq_choose = 1;
		cpufreq_set_freq_vdd_with_notify(dvfs_tbl[cpufreq_choose].clk_mcu_mhz,dvfs_tbl[cpufreq_choose].vdd_mcu_mv);
		break;
	default:
...
#ifdef CONFIG_CPU_FREQ_DEBUG
static ssize_t cpufreq_debug_show(struct kobject *kobj, struct kobj_attribute *attr,
			  char *buf)
{
    if(!cpu_freq_debug)
		return sprintf(buf,"%d:means off\n",cpu_freq_debug);
...
static ssize_t cpufreq_debug_store(struct kobject *kobj, struct kobj_attribute *attr,
			   const char *buf, size_t n)
{
	switch(buf[0]) {
	case '0':
...
{
	cpufreq_debug("****DVFS**** %s: cpufreq_bypass = 0\n",__func__);
	cpufreq_bypass = 0;
}
...
static void cpufreq_earlysuspend_late_resume(struct early_suspend *h)
{
	...
	cpufreq_debug("****DVFS**** %s: cpufreq_bypass = 1\n",__func__);
	cpufreq_bypass = 1;
	cpufreq_set_freq_vdd_with_notify(dvfs_tbl[0].clk_mcu_mhz,dvfs_tbl[0].vdd_mcu_mv);
}
