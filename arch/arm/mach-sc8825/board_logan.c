...
#if 0
static bool is_charged_full(void)
{
	int pgb, chgsb;
	bool is_full_status;

	pgb = gpio_get_value(ss6000_info.pgb)? 1 : 0;
	chgsb = gpio_get_value(ss6000_info.chgsb) ? 1 : 0;

	is_full_status = (!pgb && chgsb) ? true : false;
...
static struct spa_platform_data spa_info = {
	...
	.get_charger_full_status = is_charged_full,
#endif
}; 
...
static struct stc311x_platform_data stc3115_data = {
		...
		.Vmode= 0,       /*REG_MODE, BIT_VMODE 1=Voltage mode, 0=mixed mode */
  		...
  		.CC_cnf = 307,      /* nominal CC_cnf, coming from battery characterisation*/
  		.VM_cnf = 348,      /* nominal VM cnf , coming from battery characterisation*/
		...
		.OCVOffset2[8] = -17,      /* OCV curve adjustment */
...
#endif
#ifdef CONFIG_TOUCHSCREEN_BT432_TS
...
#endif
#ifdef CONFIG_TOUCHSCREEN_IST30XX
static struct i2c_board_info ist30xx_i2c0_boardinfo[] = {
	{
...
static struct i2c_board_info zinitix_i2c0_boardinfo[] = {
	{
...
#if defined(CONFIG_TOUCHSCREEN_IST30XX) 
	i2c_register_board_info(0, ist30xx_i2c0_boardinfo, ARRAY_SIZE(ist30xx_i2c0_boardinfo));
#else
		i2c_register_board_info(0, i2c0_boardinfo, ARRAY_SIZE(i2c0_boardinfo));
#endif
#if defined(CONFIG_TOUCHSCREEN_BT432_TS)
	i2c_register_board_info(0, zinitix_i2c0_boardinfo, ARRAY_SIZE(zinitix_i2c0_boardinfo));
#endif
	return 0;
}
...
static unsigned int headset_get_button_code_board_method(int v)
{
	...
	static struct headset_adc_range {
		...
		{ 0x0100, 0x01cf, KEY_VOLUMEUP},
		{ 0x01d0, 0x03c0, KEY_VOLUMEDOWN },
	};
...
#if defined(CONFIG_SEC_GPIO_DVS)&& defined (CONFIG_MACH_LOGANU)
   void dvs_set_init()
{      
	   gpio_direction_output(GPIO_GPS_RESET, 1);//167
	   gpio_direction_output(GPIO_BT_RESET, 1);//194
	   //gpio_direction_output(CHARGERIC_TA_INT_GPIO, 0);//201
	   //gpio_direction_output(CHARGERIC_CHG_EN_GPIO, 1);//202
}
   void dvs_set_sleep()
{
...
MACHINE_START(SC8825OPENPHONE, "sc8810i")	
