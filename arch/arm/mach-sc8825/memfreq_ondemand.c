...
//#define DELAY (5*HZ)
#define DELAY_RESUME_SET_BYPASS (msecs_to_jiffies(5000))
...
static unsigned int dbs_ask_mem(void)
{
	...
	list_for_each_entry(pos, &memfreq_dbs_handlers, link) {
		if (pos->memfreq_demand != NULL) {
			//memfreq_debug("calling %pf\n", pos->memfreq_demand);
			demand += pos->memfreq_demand(pos);
			//memfreq_debug("calling %pf done, demand:%u \n",
			//			pos->memfreq_demand, demand);
		}
	}	
	mutex_unlock(&memfreq_dbs_lock);
	return (demand >> 23);
...
static unsigned int memfreq_get_demand(void)
{
	...
	memfreq_debug("****DDRDFS**** %s: cpu_load=%u, mem_load=%u\n",__func__,cpu_load,mem_load);
...
/*after memfreq late resume , delay 5000ms to enbale DDRDFS */
static void do_memfreq_resume_set_bypass_timer(struct work_struct *work)
{
	spin_lock(&memfreq_lock);
	memfreq_bypass = 0;
	spin_unlock(&memfreq_lock);

	memfreq_debug("****DDRDFS**** %s: memfreq_bypass=%u\n",__func__,memfreq_bypass);
}

static inline void memfreq_resume_set_bypass_timer_init(void)
{
	INIT_DELAYED_WORK_DEFERRABLE(&mem_work_resume_set_bypass, do_memfreq_resume_set_bypass_timer);
}

static inline void memfreq_resume_set_bypass_timer_exit(void)
{
	cancel_delayed_work_sync(&mem_work_resume_set_bypass);
}

static inline void memfreq_resume_set_bypass_timer_start(void)
{
	memfreq_debug("****DDRDFS**** %s: memfreq_bypass=%u\n",__func__,memfreq_bypass);
	schedule_delayed_work_on(0, &mem_work_resume_set_bypass, DELAY_RESUME_SET_BYPASS);
}
...
static void do_dbs_timer(struct work_struct *work)
{
	 ...
	 */
	memfreq_curr = emc_freq_get();
	memfreq_debug("****DDRDFS**** %s: memfreq_bypass=%u,mm_is_on()=%u,memfreq_curr=%u\n",
			__func__,memfreq_bypass,mm_is_on(),memfreq_curr);
...
{
	memfreq_debug("****DDRDFS**** %s: \n",__func__);
	memfreq_resume_set_bypass_timer_start();
}
...
static int __init memfreq_gov_dbs_init(void)
{
	...
	bm_reset();
	dbs_timer_init( );
	memfreq_resume_set_bypass_timer_init();
...
{
	dbs_timer_exit( );
	memfreq_resume_set_bypass_timer_exit();
