...
void sec_debug_save_pte(void *pte, int task_addr) 
{
	...
	memcpy(&per_cpu(sec_debug_fault_status, cpu_id), pte,sizeof(struct sec_debug_fault_status_t));
	per_cpu(sec_debug_fault_status, cpu_id).cur_process_magic =task_addr;
}
