...
static __devinit int smuic_probe(struct platform_device *pdev)
{
...
static int __devexit smuic_remove(struct platform_device *pdev)
{
	printk("%s\n",__func__);
	return 0;
}
...
static struct platform_driver smuic_driver = {
        ...
        .probe          = smuic_probe,
        .remove         = __devexit_p(smuic_remove),
